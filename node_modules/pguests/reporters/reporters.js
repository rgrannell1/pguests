...
const showRemoteServer = (maxLengths, err, ipData, hostname, service) => {
	...
	/*
	console.log('\n' + commons.padString(maxLengths.ip + 4, remoteIpProfile.ip))

	const portString = commons.padString(5 + 4, port)
	const hostname   = ipMetadata.hostname ? ipMetadata.hostname : 'unknown-host'

	const conns      = commons.padString(5 + 4, remoteIpProfile.ports[port].length)
