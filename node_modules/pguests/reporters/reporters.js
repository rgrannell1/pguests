...
const showRemoteServer = (maxLengths, ipData) => {
	...
	const service  = ipData.metadata.service
	const hostname = ipData.metadata.hostname

	console.log([

			service
				? margin + commons.padString(5 + margin.length, service)
				: margin + commons.padString(5 + margin.length, ipData.port),

			margin + ipData.metadata.protocol + margin +
