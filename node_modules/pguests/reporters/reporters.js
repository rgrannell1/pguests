...
const showRemoteServer = (maxLengths, ipData) => {

	var messsage = ''
	var margin   = '   '

	if (ipData.metadata.service) {
		messsage += margin + commons.padString(5 + margin.length, ipData.metadata.service)
	} else {
	...
	messsage += margin + ipData.metadata.protocol + margin +
		(ipData.metadata.hostname ? ipData.metadata.hostname : constants.messages.unknownHost)
...
reporters.reportRates = (args, received, callback) =>{
